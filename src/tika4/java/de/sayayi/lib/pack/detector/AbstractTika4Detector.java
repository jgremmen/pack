 ...
 * <p>
 * This class is compiled against and intended to be used with Apache Tika 4. A separate, source-compatible but
 * binary-incompatible class named {@code AbstractTikaDetector} (same package, different artifact/classpath) is
 * provided for use with Apache Tika 3.
 * <p>
 ...
 */
public abstract class AbstractTika4Detector implements Detector
{
   ...
   */
  protected AbstractTika4Detector(@NotNull PackConfig packConfig, @NotNull String mimeType)
  {
   ...
   */
  @Override
  public @NotNull MediaType detect(TikaInputStream input, Metadata metadata, ParseContext context) throws IOException
  {
    if (input != null)
    {
      ...
      // shield the stream from being closed by PackInputStream, since Tika detectors must not close the stream
      // they are probing and TikaInputStream invalidates its mark once closed
      input.setCloseShield();
